...
void swap_values(int *p1, int *p2, int *p3, int *p4);
...
int main(void)
{
	...
	printf("\n\nBefore swapping\n");
	printf("1. expected = %d, old value which was returned is %d \n",expected,valueReturned);
	printf("2. var1 = %d, var2 = %d\n",var1,var2);
	
	printf("\nAfter swapping\n");
	swap_values(&valueReturned, &expected, &var1, &var2);
	printf("1. expected = %d, valueReturned = %d\n",expected,valueReturned);
	printf("2. var1 = %d, var2 = %d\n",var1,var2);
...
void swap_values(int *p1, int *p2, int *p3, int *p4)
{
	// Swap values of p1 and p2 (valueReturned and expected).
    int temp = *p1;
    *p1 = *p2;
    *p2 = temp;

    // Swap values of p3 and p4 (var1 and var2).
    temp = *p3;
    *p3 = *p4;
    *p4 = temp;
}
